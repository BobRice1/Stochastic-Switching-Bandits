...
\subsection{\textbf{Behaviour under increased uncertainty (70--30 condition):}}
\paragraph{\textbf{Cross-Validated parameter estimation across reward environments:}}
Following the modelling framework of Beron et al. (2022)\cite{Beron2022},we fit the recursive formulation of logistic regression (RFLR) separately in the 80-20 and 70-30 reward environments.
To ensure robust parameter recovery and prevent session-level leakage, a 5-fold cross-validation was performed at the session level. Sessions were randomly split into five folds with fixed random seed. For each fold, parameters \(\alpha\), \(\beta\), and \(\tau\) were estimated on four folds via maximum likelihood estimation. Optimisation was performed using stochastic gradient descent (10 000 steps, learning rate = 0.01), and log-likelihood was evaluated on the held-out fold. Final parameter estimates for each environment were computed as the mean across folds. This procedure therefore extends the fitting approach by explicitly validating generalisation across held-out sessions. 
\paragraph{\textbf{Generative model simulations:}}
Using cross-validated parameters, we generated synthetic behaviour in both reward environments under three model classes: RFLR, Ideal HMM and Sticky HMM. All simulations were performed on the true latent state sequences extracted from experimental data to maintain identical block-transition structure across models. Reward delivery probabilities were set to 0.8/0.2 or 0.7/0.3, respectively, with transition probabilities estimated from empirical block statistics. 
\paragraph{\textbf{Behavioural metrics:}}
To evaluate model performance at the macro-behavioural level, we quantified the trade-off between reward acquisition and behavioural cost using an efficiency frontier analysis. For each session, we computed total reward, total number of switches, and the reward-per-switch ratio. This extends the reward-per-switch analysis presented in the original paper \cite{Beron2022} by enabling direct comparison of behavioural efficiency across environments with differing levels of reward uncertainty. To characterise adaptation dynamics at the micro-behavioural level, we analysed event-triggered switching probability aligned to block reversals. Switching probability was computed within a window between five trials before and ten trials after each contingency reversal. This approach mirrors the block-aligned $P(\mathrm{switch})$
analysis shown in the original work \cite{Beron2022}, but extends it by explicitly contrasting switching dynamics between the 80-20 and 70-30 reward conditions, thereby quantifying environment-dependent differences in cognitive lag. Finally to asses cross-environment behavioural adaptation, we compared cross-validated parameter estimates (\(\alpha\), \(\beta\), and \(\tau\)) between the two reward conditions. In particular, we tested wether adaptation was primarily mediated through modulation of the stickiness parameter \(\alpha\), consistent with the environmental adjustments reported by Beron et al. (2022)\cite{Beron2022}. 

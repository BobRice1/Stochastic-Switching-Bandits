...
class BayesianMouse(Mouse):
    ...
    def __init__(self, params):
        ...
        # Stickiness Initialisation
        self.alpha = params.get('alpha', 0.0) # Stickiness weight
        self.beta = params.get('beta', 0.0)
        self.tau = params.get('tau', 1.0)     # Decaying memory timescale
        self.stickiness = 0.0                 # Current kappa bias
        
        # Pre-calculate equivalence constants 
        self.decay = np.exp(-1.0 / self.tau)
        self.s1 = self.alpha + (self.beta / 2.0)
        self.s2 = -self.alpha * self.decay
    ...
    def make_choice(self, policy):
        ...
        if policy=='greedy':
            ...
            # Combined Bayesian + Stickiness Policy
            # P(choice) = sigmoid( log-odds(belief) + stickiness_bias )
            psi = logit(prediction[1]) + self.stickiness
            return np.random.rand() < sigmoid(psi)
    
    def update_stickiness(self, last_choices):
        """
        Calculates stickiness (kappa) bias recursively
        last_choices should be the array [choice_{t-1}, choice_t]
        """
        # Handle the very first trial where there is no c_{t-1}
        if len(last_choices) == 1:
            c_t = 2 * last_choices[0] - 1
            self.stickiness = self.s1 * c_t
        else:
            # Recode choices to -1 (Right) and 1 (Left)
            c_t = 2 * last_choices[-1] - 1
            c_t_minus_1 = 2 * last_choices[-2] - 1
            
            # Recursive stickiness update
            self.stickiness = (self.decay * self.stickiness) + (self.s1 * c_t) + (self.s2 * c_t_minus_1)

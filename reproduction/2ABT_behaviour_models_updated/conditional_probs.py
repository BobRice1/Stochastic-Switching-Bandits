...
def add_history_cols(df, N):
    ...
    df['decision_seq'] = None # initialize column for decision history (current trial excluded)
    df['reward_seq'] = None # initialize column for laser stim history (current trial excluded)
    df['decision_seq'] = df['decision_seq'].astype(object)
    df['reward_seq'] = df['reward_seq'].astype(object)
...
def calc_conditional_probs(df, symm, action=['Switch'], run=0):
    ...
    conditional_probs = df.groupby(group).agg(
        paction=pd.NamedAgg(column=action[run], aggfunc="mean"), # changed np.mean to "mean"
        n = pd.NamedAgg(column=action[run], aggfunc=len),
    ).reset_index()
